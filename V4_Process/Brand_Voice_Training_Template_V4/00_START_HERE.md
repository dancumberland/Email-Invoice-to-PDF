...
3. **Run the *Source Ingestion Prompt*** (see `03_INGESTION_GUIDE.md`) for ≤3 chunks at a time.  
   – After you ingest a chunk, mark its Status **🟡 Read** in the Process Tracker.  
   – Once you’ve integrated its insights into the Master Guide, change that row to **✅ Applied**.
...
3. Let the AI iterate through all, populating the 17-section structure.
...
├─ Summaries/         # (unused in Read → Apply workflow)
