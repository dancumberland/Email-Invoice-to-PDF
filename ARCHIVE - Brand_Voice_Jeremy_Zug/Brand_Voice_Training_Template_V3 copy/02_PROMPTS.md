...
```
Synthesize information from the client's OLD template files: `01_executive_summary.md`, `02_brand_foundation.md`, and relevant voice-informing parts of `03_brand_assets_and_history.md` to create the 'Core Brand Identity' section for the NEW Master Brand Voice Guide.
...
```
Condense and synthesize information from the client's OLD template files: `05_linguistic_patterns.md`, key elements of `08_brand_language_parameters.md`, and relevant voice/tone descriptions from `02_brand_foundation.md` to create the 'Voice, Tone, And Style' section for the NEW Master Brand Voice Guide.
...
```
Synthesize key principles from the client's OLD template files: `04_content_generation_guidelines.md` (focus on general principles, not deep platform-specific details) and `06_audience_engagement.md` (extract audience summaries and core engagement principles relevant to voice) to create the 'Content Strategy And Audience' section for the NEW Master Brand Voice Guide.
...
```
Synthesize information from the client's OLD template files: `07_advanced_storytelling.md` (extract 1-2 core story archetypes if truly central and distinct) and, most importantly, identify 2-4 *prime examples of actual client content* that best demonstrate the brand voice in action. Create the 'Key Exemplars And Storytelling' section for the NEW Master Brand Voice Guide.
...
Synthesize information from the client's OLD template files, particularly `04_content_generation_guidelines.md` (for platform-specific strategies) and any other files that contain [Platform Name]-specific examples or instructions.
...
*   When synthesizing from "OLD template files," refer to the original, detailed markdown files provided by the client (e.g., `01_executive_summary.md`, `04_content_generation_guidelines.md`, etc.).
*   Adhere strictly to the conciseness and example guidelines outlined in `00_AI_INSTRUCTIONS.md` (Version 2.0).
