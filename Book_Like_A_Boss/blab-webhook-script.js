...
function doPost(e) {
  try {
    ...
    const eventType = extractField(data, ['event', 'event_type', 'webhook_event', 'trigger', 'action']) || 'booking';
    ...
    sendSlackNotification(clientName.trim(), packageName, bookingDatetime, eventType, bookingId);
 ...
 */
function sendSlackNotification(name, service, datetime, eventType, bookingId) {
...
function processKitSubscriptions() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const success = addToKit(email.toString().trim(), firstName);
 ...
 */
function addToKit(email, firstName) {
  ...
  try {
    ...
    if (code === 200 || code === 201) {
      ...
      return addTagToExistingSubscriber(email);
    } else {
 ...
 * @returns {boolean} - Success status
 */
function addTagToExistingSubscriber(email) {
  ...
  try {
    ...
    if (findData.subscribers && findData.subscribers.length > 0) {
      ...
      const updatePayload = { fields: { kit_last_form: 'BLAB Networking Call' } };
